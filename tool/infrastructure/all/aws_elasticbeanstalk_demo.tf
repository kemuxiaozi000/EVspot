...
locals {
  ...
  demo_rails_env = "development"
  demo_eb_bucket_name_full = "${format("%s-app-deployments-%s-%s", var.app_name, local.demo_environment, var.account_id)}"
  demo_eb_bucket_name = "${substr(local.demo_eb_bucket_name_full, 0 , min(63, length(local.demo_eb_bucket_name_full)))}"
}

resource "aws_s3_bucket" "aws_eb_s3_demo" {
  bucket = "${local.demo_eb_bucket_name}"
...
resource "aws_elastic_beanstalk_environment" "aws_eb_web_app_demo_env" {
  name         = "${var.app_name}-${local.demo_environment}"
  application  = "${aws_elastic_beanstalk_application.aws_eb_web_app.name}"
  cname_prefix = "${var.app_name}-${var.account_id}-${local.demo_environment}"
  tier         = "WebServer"
...
#  }
  ...
  setting {
    ...
    name      = "RAILS_ENV"
    value     = "${local.demo_rails_env}"
  }
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "ENVIRONMENT"
    value     = "${local.demo_environment}"
  }
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
  ...
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
    name      = "DB_NAME"
    value     = "${var.app_name}-${local.demo_environment}"
  }
  setting {
    namespace = "aws:elasticbeanstalk:application:environment"
